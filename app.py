...
with col_right:
    ...
    if total_sessions > 0:
        ...
        fig_pie = go.Figure(data=[go.Pie(
            ...
            hole=0.45,
            marker=dict(colors=["#2e7d32", "#66bb6a", "#a5d6a7", "#1b5e20", "#4caf50"]),
...
recent_rows = []
for name, label in [("ball_striking", "Ball Striking"), ("putting", "Putting"), ("short_game", "Short Game"), ("testing", "Short Game Testing"), ("three_hole_loop", "3-Hole Loop")]:
    df = load_csv(name)
    if not df.empty:
        df = df.copy()
        df["category"] = label
